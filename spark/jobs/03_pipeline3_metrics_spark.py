...
def main():
    ...
    med4 = df4.groupBy("product_id").agg(
        percentile_approx("price", 0.5).alias("median_4w"),
    ...
    first_last = (
        ...
        .withColumn(
            "trend_2w", (col("last_price") - col("first_price")) / col("first_price")
        )
    )

    out = min12.join(med4, "product_id", "outer").join(
        first_last.select("product_id", "trend_2w"), "product_id", "outer"
    )
    rows = [r.asDict() for r in out.collect()]
    for r in rows:
        r["computed_at"] = now
        db.metrics_snapshot.update_one(
            {"product_id": r["product_id"]}, {"$set": r}, upsert=True
        )
