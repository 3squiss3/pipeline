...
def main():
    db = get_db()
    for c in ["products","products_curated","price_events","metrics_snapshot","product_views","alerts","pipeline_state"]:
        db[c].delete_many({})
    ...
    now = datetime.utcnow()
    db.products_curated.insert_many([{**p, "banner":"Surveiller", "updated_at": now} for p in products])
